...
describe('identifyRequirementsForWorkflows', () => {
	it('returns undefined when there are no requirements', () => {
		expect(identifyRequirementsForWorkflows(undefined, [prepared('W1')])).toBeUndefined();
	});
	...
	it('keeps only in-scope workflows and drops requirements that no in-scope workflow uses', () => {
		...
		const scoped = identifyRequirementsForWorkflows(requirements, [prepared('W1')]);
